...
1. ТРЕБОВАНИЕ К ДЛИНЕ ВОЛОС И ОБЛЫСЕНИЮ (КРИТИЧЕСКИ ВАЖНО):
   ...
   - Если клиент "Лысый" или имеет выраженную обширную лысину: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО рекомендовать густые длинные прически! Предлагай только:
     ...
     3. "Head shave with stubble / Scalp micropigmentation" (Бритье головы с эффектом 3-дневной щетины или трихопигментацией).
...
2. ТРЕБОВАНИЕ К ГУСТОТЕ И ЗАЛЫСИНАМ:
   Если у клиента редкие/тонкие волосы ("Редкие/Тонкие") или в hairlineStatus указаны залысины (temporal thinning, receding hairline, M-shape, bald spots):
   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО предлагать объемные прически с открытым зачесом назад или наверх (например, классический Помпадур, зачес Slick Back, высокий Квифф, высокие объемные укладки) — они сделают акцент на залысинах и оголят поредение!
   - КАТЕГОРИЧЕСКИ РЕКОМЕНДУЕТСЯ предлагать текстурированные стрижки с челкой вперед или вбок, нивелирующие залысины (например, Текстурированный Кроп / French Crop, стрижка Цезарь / Caesar Cut, короткая текстурированная стрижка с хаотичной укладкой вперед, Buzz Cut с переходом), либо бритье налысо.
   - Стрижки должны быть физически реализуемыми на текущей густоте клиента.
...
4. ТРЕБОВАНИЕ К СТИЛЮ:
   Ожидаемый стиль стрижки: \${preferredStyle !== undefined && preferredStyle !== 'Любой' ? preferredStyle : 'На твое усмотрение'}. Все 3 стрижки должны соответствовать этому стилю, но быть абсолютно разными и креативными.

5. ТРЕБОВАНИЕ К 'imageKeyword' (ДЛЯ ОПТИМАЛЬНОЙ ГЕНЕРАЦИИ РЕФЕРЕНСОВ):
   - 'imageKeyword' должен быть детальным набором ключевых слов ТАКЖЕ НА АНГЛИЙСКОМ, через запятую.
   - Он должен обязательно отражать возраст и пол клиента, а также правильную густоту волос и линию роста, чтобы фото-референс выглядел реалистично для этого клиента!
   - Если клиент пожилая женщина (например, 70 лет): ключевые слова ДОЛЖНЫ содержать "elderly woman, 70 years old, realistic grey hair cut, pixie style, photorealistic".
   - Если у клиента редкие волосы с залысинами: "textured french crop haircut, short forward-swept bangs, thin hair texture, slight receding hairline, middle-aged man, photorealistic".
   - Если у клиента лысина: "fully shaved head, clean scalp, stubble beard, bald man face, photorealistic".
