...
import { RefreshCw, Wand2, Upload, BookOpen, X, Image as ImageIcon } from "lucide-react";
...
export const HaircutList: React.FC<HaircutListProps> = ({
  results,
  generationsLeft,
  teaserUrl,
  isGeneratingTeaser,
  ...
  return (
    <div className="mt-4">
      ...
      {isLibraryOpen && !cropImageSrc && (
        <div className={`fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300 ${isLightMode ? 'bg-black/20' : 'bg-black/80'} backdrop-blur-sm`}>
          <div className={`w-full max-w-lg max-h-[85vh] sm:max-h-[80vh] flex flex-col rounded-t-3xl sm:rounded-3xl shadow-2xl relative animate-in slide-in-from-bottom-10 sm:slide-in-from-bottom-4 duration-300 ${isLightMode ? 'bg-white border-gray-200' : 'bg-[#1a1625] border border-white/10'}`}>
            <div className={`p-5 flex justify-between items-center border-b ${isLightMode ? 'border-gray-200' : 'border-white/10'}`}>
              <h3 className={`font-serif text-xl tracking-tight flex items-center gap-2 ${isLightMode ? 'text-gray-900' : 'text-white'}`}>
                <BookOpen size={20} className={isLightMode ? 'text-indigo-600' : 'text-indigo-400'} />
                Свой референс
              </h3>
              <button 
            ...
            <div className="overflow-y-auto p-5 custom-scrollbar">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className={`text-xs font-semibold uppercase tracking-wider ${isLightMode ? 'text-gray-400' : 'text-white/40'}`}>Или из нашей базы</span>
                <div className={`h-px flex-1 ${isLightMode ? 'bg-gray-200' : 'bg-white/10'}`}></div>
              </div>

              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {(results?.gender === 'female' ? FEMALE_LIBRARY : MALE_LIBRARY).map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => {
                      setTryOnStyle({
