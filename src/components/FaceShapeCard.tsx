...
export const FaceShapeCard: React.FC<FaceShapeCardProps> = ({ results, isLightMode }) => {
  return (
    <div className="flex flex-col gap-3">
      <div className={`flex items-center gap-2 px-3 py-1.5 w-fit rounded-full border ${isLightMode ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300'}`}>
        <Cpu size={14} />
        <span className="text-[10px] md:text-xs font-medium tracking-wide uppercase">Локальная нейросеть (Приватность 100%)</span>
      </div>
