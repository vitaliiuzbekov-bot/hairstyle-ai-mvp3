...
export default function App() {
  ...
  return (
    ...
    >
                        ...
                        <div className="w-full aspect-[4/5] relative">
                          {idx === 0 && (generationsLeft === null || generationsLeft <= 0 || teaserUrl) ? (
                            ...
                            <>
                              ...
                              <div className="absolute top-3 left-3 w-8 h-8 rounded-full bg-white/5 backdrop-blur-md flex items-center justify-center text-white/90 text-xs font-medium border border-white/10 z-20 shadow-sm">
                                {idx + 1}
                              </div>
                            </>
                          )}
              ...
              <div className="flex-1 lg:pl-8 lg:border-l border-white/10 order-1 lg:order-2">
                ...
                <div className="mt-6 flex flex-col gap-3">
                  ...
                  <div className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-3">
                    ...
                    {vtonResultUrl && isTeaserResult && (
                      <div className="mb-4 relative rounded-2xl overflow-hidden border border-white/10 bg-black/40">
                    ...
                    {vtonResultUrl && (customHairColor || results?.hairColor) && COLOR_BRANDS[customHairColor || results?.hairColor || ""] && !isTeaserResult && (
                      <div className="mb-4 bg-purple-500/10 border border-purple-500/20 rounded-2xl p-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {COLOR_BRANDS[customHairColor || results?.hairColor || ""].map((brand, bIdx) => (
                            <div key={bIdx} className="bg-white/5 border border-white/10 rounded-xl p-3 flex justify-between items-center">
                              <span className="text-sm text-white/80">{brand.name}</span>
                              <span className="text-sm font-mono text-purple-300 bg-purple-500/20 px-2 py-1 rounded-md">{brand.shade}</span>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-[#111] border border-white/10 rounded-3xl p-6 shadow-2xl relative flex flex-col items-center">
            ...
            <div className="flex flex-col gap-4 w-full">
              {[
                ...
                <button
                  ...
                  disabled={isBuying}
                  className={`relative flex items-center justify-between w-full p-4 rounded-2xl bg-white/5 border ${pkg.isPopular ? 'border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-white/10'} hover:bg-white/10 active:scale-[0.98] transition-all disabled:opacity-50 overflow-hidden`}
                >
                  {pkg.isPopular && (
                  ...
                  <div className="flex flex-col items-start gap-1 z-10 w-2/3">
                     <span className="font-bold text-white/90 text-sm sm:text-base text-left leading-tight">{pkg.label}</span>
                     <span className="text-xs text-white/50">{pkg.count} генераци{pkg.count === 1 ? 'я' : pkg.count < 5 ? 'и' : 'й'}</span>
                  </div>
                  <div className="flex items-center gap-1.5 bg-amber-500/20 px-3 py-1.5 rounded-full border border-amber-500/30 z-10 shrink-0">
            ...
            <div className="mt-8 pt-6 border-t border-white/10 w-full text-center">
              <h3 className="text-white/90 font-medium mb-2 flex items-center justify-center gap-2">
              ...
              <button 
                ...
                className="w-full bg-white/5 hover:bg-white/10 border border-white/10 text-white/90 text-sm py-3 rounded-2xl flex items-center justify-center gap-2 transition-all active:scale-95"
              >
      ...
      {showWelcome && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg bg-[#111] border border-white/10 rounded-3xl shadow-2xl flex flex-col">
            <div className="p-6 pt-8 text-center border-b border-white/5">
              <div className="mx-auto w-16 h-16 bg-gradient-to-tr from-blue-500/20 to-purple-500/20 rounded-full flex items-center justify-center mb-4 border border-white/10">
                <Scissors size={32} className="text-white/80" />
              </div>
              <h2 className="text-2xl font-bold text-white tracking-tight">Добро пожаловать в НейроСтилист</h2>
              <p className="text-white/50 text-sm mt-2">Кто вы?</p>
            </div>
            
            <div className="p-6 space-y-3">
              {showSalonNameInput ? (
                <div className="flex flex-col gap-4 animate-in fade-in zoom-in duration-300">
                  <h3 className="text-lg font-medium text-white/90 text-center">Укажите название вашего салона</h3>
                  <input
                    type="text"
                    value={salonName}
                    onChange={(e) => setSalonName(e.target.value)}
                    placeholder="Например: Салон 'Красота'"
                    className="w-full bg-[#1A1A1A] text-white p-4 rounded-xl border border-white/10 focus:border-purple-500/50 outline-none transition-colors placeholder:text-white/30 text-center"
                  />
                  <button 
                    onClick={() => {
                        setUserRole('salon');
                  ...
                  <button onClick={() => setShowSalonNameInput(false)} className="text-white/50 text-sm hover:text-white/80 transition-colors">
                    Назад
                  </button>
                </div>
              ) : (
                <>
                  <button 
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 active:scale-[0.98] transition-all group flex items-center gap-4"
                  >
                    <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center shrink-0 border border-blue-500/30 group-hover:scale-110 transition-transform">
                      <User size={24} className="text-blue-400" />
                    </div>
                    <div>
                      <h3 className="font-bold text-white/90">Ищу свой стиль</h3>
                      <p className="text-sm text-white/50 mt-0.5">Хочу подобрать идеальную стрижку и цвет волос для себя</p>
                    </div>
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 active:scale-[0.98] transition-all group flex items-center gap-4"
                  >
                    <div className="w-12 h-12 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0 border border-amber-500/30 group-hover:scale-110 transition-transform">
                      <Scissors size={24} className="text-amber-400" />
                    </div>
                    <div>
                      <h3 className="font-bold text-white/90">Я стилист / мастер</h3>
                      <p className="text-sm text-white/50 mt-0.5">Буду показывать клиентам варианты до стрижки</p>
                    </div>
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full text-left p-4 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 active:scale-[0.98] transition-all group flex items-center gap-4"
                  >
                    <div className="w-12 h-12 rounded-full bg-purple-500/20 flex items-center justify-center shrink-0 border border-purple-500/30 group-hover:scale-110 transition-transform">
                      <Store size={24} className="text-purple-400" />
                    </div>
                    <div>
                      <h3 className="font-bold text-white/90">Я владелец салона</h3>
                      <p className="text-sm text-white/50 mt-0.5">Хочу внедрить этот инструмент для своих мастеров</p>
                    </div>
                  </button>
                </>
              )}
