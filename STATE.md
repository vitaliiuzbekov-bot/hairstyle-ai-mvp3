...
23. **Исправление отображения баланса (баг жесткого мока 999 генераций)** - В `useTokenManager.ts` для аккаунтов разработчика/тестировщика стояло принудительное переопределение `isDevUser ? 999 : data.generationsLeft`, а также в `onSnapshot` стояло условие `if (!isDeveloper)`, блокировавшее реалтайм-обновление счетчика из Firestore. Из-за этого при покупке и начислении в боте баланс в базе обновлялся корректно (+10), но на клиенте визуально оставалась заглушка 999. Мок полностью удален, интерфейс теперь отображает реальный баланс из базы данных и синхронизируется в реальном времени.

## Текущая задача
- Все исправления применены, приложение и Telegram бот протестированы и готовы к работе.
