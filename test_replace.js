...
analysisRouter.get("/job/:jobId", (req, res) => {
...
analysisRouter.post("/analyze", analyzeLimiter, async (req: Request, res: Response): Promise<void> => {
  try {
    const jobId = req.body.idempotencyKey || require('crypto').randomUUID();
    ...
    if (cachedAnalysis) {
        ...
        analyzeJobMap.set(jobId, { status: "completed", result: cachedAnalysis });
        return;
    }
